...
function emplace(map, key, handler) {
  ...
  if (!handler.insert)
    throw new Error(
      false ? formatProdErrorMessage(10) : 'No insert provided for key not already in map'
    );
  ...
  const stopListening = (options) => {
    ...
    const entry = find(Array.from(listenerMap.values()), (entry2) => {
      const matchPredicateOrType =
        typeof type === 'string' ? entry2.type === type : entry2.predicate === predicate;
  ...
  const selector = Object.assign(
    function makeSelector(selectorFn, selectState) {
      return function selector2(state, ...args) {
        return selectorFn(
          createStateProxy(selectState ? selectState(state, ...args) : state, reducerMap),
          ...args
        );
      };
    },
