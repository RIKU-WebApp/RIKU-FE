...
const FlashRunAdmin: React.FC<FlashRunAdminData> = ({
  ...
  const handleModalStartClick = async () => {
    ...
    try {
      ...
      //  출석 상태 먼저 반영
      await customAxios.patch(`/run/flash/post/${postId}/manual-attendance`, attendanceData, {
  ...
  const handleTabChange = async (tab: '소개' | '명단') => {
    ...
    try {
      ...
      if (response.data.isSuccess) {
        ...
        // 탭 별 업데이트
        if (tab === '명단') {
          const converted = result.participants.map((p: EditableUser) => ({
            userId: p.userId,
            userName: p.userName,
            userProfileImg: p.userProfileImg || null,
            status:
              p.status === 'ATTENDED' ? 'ATTENDED' : p.status === 'ABSENT' ? 'ABSENT' : 'PENDING',
          }));
  ...
  useEffect(() => {
    if (participants.length > 0) {
      ...
      setEditableParticipants(converted as EditableUser[]);
    }
  }, [postId, participants]);
  ...
  const refetchPost = async () => {
    try {
      ...
      if (data.isSuccess) {
        ...
        // 참가자/카운트/상태 등 최신화
        const converted = r.participants.map((p: EditableUser) => ({
          userId: p.userId,
          userName: p.userName,
          userProfileImg: p.userProfileImg || null,
          status:
                ...
                <motion.button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const { data } = await customAxios.delete(`/run/flash/post/${postId}`, {
                        headers: { Authorization: `${token}` },
                      });
        ...
        <div className="relative w-full max-w-[430px] pb-[10px]">
          <div className="relative w-full overflow-hidden">
            <img
              ...
              className={`w-full h-[308px] object-cover transition-all duration-300 ${
                showMenu || postStatus === 'CANCELED' || postStatus === 'CLOSED'
                  ? 'brightness-75'
                  : ''
              }`}
            />
